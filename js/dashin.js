document.addEventListener("DOMContentLoaded", function () {
    ...
    // Hapus semua bagian yang terkait dengan upload
    ...
    // Fungsi untuk mendapatkan status progress
    ...
    function renderRow(row, index, id) {
      ...
      return `
        <tr data-id="${id}">
          ...
          <td>${row["TIME IN"] === "-" ? "" : (row["TIME IN"] || "")}</td>
          <td>${row["UNLOADING TIME"] === "-" ? "" : (row["UNLOADING TIME"] || "")}</td>
          <td>${row["FINISH"] === "-" ? "" : (row["FINISH"] || "")}</td>
        </tr>
      `;
    }
  
    // Load data dari Airtable
    ...
    // ✅ Pindahkan auto-refresh ke sini:
    setInterval(() => {
